...
void problema_1::inserareInFata(List*& myList_, int numar, int pozitie)
{
	...
	if (pozitie == 0)
	{
		if (myList_->prev == NULL)
		{
			List* newElement = new List{ numar,NULL,myList_ };
			myList_->prev = newElement;
			myList_ = newElement;
			return;
		}
		else
		{
			List* newElement = new List{ numar,myList_ ,NULL};
			myList_->next = newElement;
			myList_ = newElement;
			return;
		}
	}
...
problema_2::List* problema_2::lipire(problema_2::List* X, problema_2::List* Y)
{
	List* myList = NULL;
	List* iter;
	if (X->next == 0)
...
problema_2::List* problema_2::interclasare(problema_2::List* X, problema_2::List* Y)
{
	if (X == NULL)
	{
		return Y;
